...
import { RefreshCw, Search, Video, Wifi, WifiOff, HardDrive } from "lucide-react";
...
export default function NvrPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter((row) => [row.name, row.host, row.site, row.model, row.serialNumber].some((value) => value?.toLowerCase().includes(q)));
  }, [rows, search]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">NVR, CCTV channel and storage status.</p>
        </div>
      ...
      <div className="rounded-xl border bg-background p-4 shadow-sm">
        <div className="relative max-w-xl">
          ...
          <Input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search NVR / IP / Site / Model..." className="pl-9" />
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1050px] text-sm">
            <thead className="border-b bg-muted/40 text-left text-xs text-muted-foreground">
              <tr><th className="px-4 py-3">NVR</th><th className="px-4 py-3">IP</th><th className="px-4 py-3">Site</th><th className="px-4 py-3">Model</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">CCTV</th><th className="px-4 py-3">Storage</th><th className="px-4 py-3 text-right">Action</th></tr>
            </thead>
            <tbody className="divide-y">
              {loading ? <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">Loading Hikvision NVR...</td></tr> : visible.length === 0 ? <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">No NVR found</td></tr> : visible.map((row) => (
...
function SummaryCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: React.ReactNode }) {
  return <div className="rounded-xl border bg-background p-4 shadow-sm"><div className="flex items-center gap-2 text-xs text-muted-foreground">{icon}{label}</div><div className="mt-2 text-2xl font-bold tabular-nums">{value}</div></div>;
}

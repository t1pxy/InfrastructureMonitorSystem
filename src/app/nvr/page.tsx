...
import { ArrowRight, HardDrive, RefreshCw, Search, Settings, Video, Wifi, WifiOff } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import type { Nvr, NvrConfig, NvrListResponse } from "@/types/nvr";
import { NvrConfigDialog } from "@/components/hikvision/NvrConfigDialog";
...
export default function NvrPage() {
  ...
  const [configs, setConfigs] = useState<NvrConfig[]>([]);
  const [editing, setEditing] = useState<NvrConfig | null>(null);
  ...
  async function load() {
    try {
      ...
      if (configResponse.ok && configJson.success) setConfigs(configJson.data.nvrs);
    } catch (cause) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) =>
      [row.name, row.host, row.site, row.model, row.serialNumber].some((value) => value?.toLowerCase().includes(q)),
    );
  }, [rows, search]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border bg-background p-4 shadow-sm"><div className="relative max-w-xl"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Search NVR / IP / Site / Model..." className="pl-9" /></div></div>
      {error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700"><b>Cannot load NVR:</b> {error}</div> : null}

      <div className="overflow-hidden rounded-xl border bg-background shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-sm"><thead className="border-b bg-muted/40 text-left text-xs text-muted-foreground"><tr><th className="px-4 py-3">NVR</th><th className="px-4 py-3">IP</th><th className="px-4 py-3">Site</th><th className="px-4 py-3">Model</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">CCTV</th><th className="px-4 py-3">Storage</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y">
        {loading ? <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">Loading Hikvision NVR...</td></tr> : visible.length === 0 ? <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">No NVR found</td></tr> : visible.map((row) => <tr key={row.id} className="hover:bg-muted/20">
          <td className="px-4 py-3"><div className="font-medium">{row.name}</div><div className="text-xs text-muted-foreground">{row.serialNumber ?? "Serial unavailable"}</div></td>
          <td className="px-4 py-3 font-mono text-xs">{row.host}</td><td className="px-4 py-3">{row.site ?? "-"}</td><td className="px-4 py-3">{row.model ?? "-"}</td>
          ...
          <td className="px-4 py-3 text-right"><div className="flex justify-end gap-2"><Button variant="outline" size="sm" onClick={() => { setEditing(configs.find((item) => item.id === row.id) ?? { id: row.id, name: row.name, host: row.host, username: "", password: "", site: row.site ?? undefined }); setConfigOpen(true); }}><Settings className="mr-1.5 h-3.5 w-3.5" />Edit</Button><Link href={`/nvr/${row.routeId}`} className="inline-flex h-8 items-center justify-center rounded-lg border bg-background px-2.5 text-sm font-medium transition-colors hover:bg-muted">View CCTV<ArrowRight className="ml-1.5 h-3.5 w-3.5" /></Link></div></td>
        </tr>)}
      </tbody></table></div></div>

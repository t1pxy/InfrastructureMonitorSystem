...
function durationSince(value: string | null | undefined) {
...
export default function NvrDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    <div className="flex items-center justify-between gap-4"><Link href="/nvr" className="inline-flex h-8 items-center justify-center rounded-lg px-2.5 text-sm font-medium transition-colors hover:bg-muted"><ArrowLeft className="mr-2 h-4 w-4" /> NVR Monitor</Link><Button variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Refresh</Button></div>
    {error ? <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div> : null}
    {nvr ? <>
      <div className="rounded-xl border bg-background p-5 shadow-sm"><div className="flex flex-col justify-between gap-4 md:flex-row"><div><h1 className="text-2xl font-bold">{nvr.name}</h1><p className="mt-1 font-mono text-sm text-muted-foreground">{nvr.host}</p></div><span className={`h-fit rounded-full px-3 py-1 text-xs font-semibold ${nvr.status === "ONLINE" ? "bg-emerald-100 text-emerald-700" : nvr.status === "OFFLINE" ? "bg-red-100 text-red-700" : "bg-muted text-muted-foreground"}`}>{nvr.status}</span></div><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><Info label="Site" value={nvr.site ?? "-"} /><Info label="Model" value={nvr.model ?? "-"} /><Info label="Serial" value={nvr.serialNumber ?? "-"} /><Info label="Firmware" value={nvr.firmware ?? "-"} /><Info label="Last Check" value={formatThai(nvr.lastChecked)} /></div></div>
